...
#include "hier-parser-node-info.hpp"
      ...
      for (it = m_consumer_nodes.begin (); it != m_consumer_nodes.end (); ++it)
        {
          if (it->Name == row_info.at (0))
            {
              std::string data_prefix = generate_data_prefix (row_info.at (1));
              it->data_prefix_provided.push_back (data_prefix);
      ...
      if (it == m_consumer_nodes.end ())
        {
          ...
          new_node.Name = row_info.at (0);
          std::string data_prefix = generate_data_prefix (row_info.at (1));
          new_node.data_prefix_provided.push_back (data_prefix);
...
std::string
parserNodeInfo::generate_data_prefix (std::string data_name)
{
  //std::cout<<"data_name is "<<data_name<<std::endl;
  std::vector<std::string> data_prefix_list;
  std::vector<std::string> input_data;
  std::string data_prefix = "/Data";
  data_prefix.append ("/" + data_name);
  //std::cout<<"data prefix is "<<data_prefix<<std::endl;
  return data_prefix;
}
} //namespace inc
} //namespace ndn
