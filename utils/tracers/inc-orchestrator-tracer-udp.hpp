...
#ifndef SRC_INC_ORCHESTRATOR_UDP_COMMUNICATION_TRACER_HPP_
#define SRC_INC_ORCHESTRATOR_UDP_COMMUNICATION_TRACER_HPP_
...
/// @cond include_hidden
struct RequestStats_udp_orchestrator {
  inline void
  Reset()
  {
    m_OrchestrationInterestSendOut = 0;
    m_OrchestrationDataReceived = 0;
  }
  ...
  __int32_t m_OrchestrationInterestSendOut;              /// @brief counter to measure Interest sent by the node in the particular time interval
  __int32_t m_OrchestrationDataReceived;                 /// @brief counter to measure Data received by the node in the particular time interval
  __int32_t m_OrchestrationDecisionEnforced;             /// @brief counter to measure orchestration management messages sent by the node in the particular time interval
  double m_overallOrchestrationInterestSend = 0;         /// @brief counter to measure all Interest sent by the node
  double m_overallOrchestrationDecisionsSend = 0;        /// @brief counter to measure all decisions sent by the node
  double m_overallOrchestrationDataReceived = 0;         /// @brief counter to measure all Data received by the node
  std::vector<std::string> m_orchestrationDecisions;      ///@brief list of names of orchestration decisions enforced by the orchestrator
 ...
 */
class IncOrchestratorCommunicationTracerUdp : public SimpleRefCount<IncOrchestratorCommunicationTracerUdp> {
public:
  /**
   ...
   * tuple needs to be preserved for the lifetime of simulation, otherwise SEGFAULTs are inevitable
   */
  static Ptr<IncOrchestratorCommunicationTracerUdp>
   ...
   */
  IncOrchestratorCommunicationTracerUdp(std::shared_ptr<std::ostream> os, Ptr<Node> node);
   ...
   */
  IncOrchestratorCommunicationTracerUdp(std::shared_ptr<std::ostream> os, const std::string& node);

  /**
   * @brief Destructor
   */
  ~IncOrchestratorCommunicationTracerUdp();
...
private:
  ...
  void
  SendOrchestrationInterests(ns3::Ptr<const Packet>);

  void
  IncomingOrchestrationDatas(ns3::Ptr<const Packet>);

  void
  SendOrchestrationDecisions(ns3::Ptr<const Packet>);
...
private:
  ...
  RequestStats_udp_orchestrator m_stats;
};
 ...
 */
inline std::ostream&
operator<<(std::ostream& os, const IncOrchestratorCommunicationTracerUdp& tracer)
{
  os << "# ";
...
#endif /* SRC_INC_ORCHESTRATOR_UDP_COMMUNICATION_TRACER_HPP_ */
