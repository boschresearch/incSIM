...
#include "inc-orchestrator-tracer.hpp"
...
NS_LOG_COMPONENT_DEFINE("IncOrchestratorCommunicationTracer");

namespace ns3 {
namespace ndn {
namespace inc {
static std::list<std::tuple<shared_ptr<std::ostream>, std::list<Ptr<IncOrchestratorCommunicationTracer>>>> g_tracers;

void
IncOrchestratorCommunicationTracer::Destroy()
{
  g_tracers.clear();
}

void
IncOrchestratorCommunicationTracer::InstallAll(const std::string& file, Time averagingPeriod /* = Seconds (0.5)*/)
{
  using namespace boost;
  using namespace std;

  std::list<Ptr<IncOrchestratorCommunicationTracer>> tracers;
  ...
  for (NodeList::Iterator node = NodeList::Begin(); node != NodeList::End(); node++) {
    Ptr<IncOrchestratorCommunicationTracer> trace = Install(*node, outputStream, averagingPeriod);
...
void
IncOrchestratorCommunicationTracer::Install(const NodeContainer& nodes, const std::string& file,
...
void
IncOrchestratorCommunicationTracer::Install(Ptr<Node> node, const std::string& file,
                  Time averagingPeriod /* = Seconds (0.5)*/)
{
  using namespace boost;
  using namespace std;
...
Ptr<IncOrchestratorCommunicationTracer>
IncOrchestratorCommunicationTracer::Install(Ptr<Node> node, shared_ptr<std::ostream> outputStream,
                  Time averagingPeriod /* = Seconds (0.5)*/)
{
  NS_LOG_DEBUG("Install tracer on node: " << node->GetId());
  Ptr<IncOrchestratorCommunicationTracer> trace = Create<IncOrchestratorCommunicationTracer>(outputStream, node);
...
IncOrchestratorCommunicationTracer::IncOrchestratorCommunicationTracer(shared_ptr<std::ostream> os, Ptr<Node> node)
  : m_nodePtr(node)
  , m_os(os)
{
  NS_LOG_DEBUG("IncOrchestratorCommunicationTracer - Constructor");
...
IncOrchestratorCommunicationTracer::IncOrchestratorCommunicationTracer(shared_ptr<std::ostream> os,
...
IncOrchestratorCommunicationTracer::~IncOrchestratorCommunicationTracer(){

  NS_LOG_DEBUG("Destructor IncOrchestratorCommunicationTracer");
...
void
IncOrchestratorCommunicationTracer::Connect()
{
  ...
  for(int i = 0; i<num_apps ; i++)
  {
    ...
    orchestrator_app.at(i)->TraceConnectWithoutContext("SendOrchInterests",
        MakeCallback(&IncOrchestratorCommunicationTracer::SendOrchestrationInterests, this));
    orchestrator_app.at(i)->TraceConnectWithoutContext("IncomingOrchDatas",
       MakeCallback(&IncOrchestratorCommunicationTracer::IncomingOrchestrationDatas, this));
    orchestrator_app.at(i)->TraceConnectWithoutContext("SendOrchDecisions",
        MakeCallback(&IncOrchestratorCommunicationTracer::SendOrchestrationDecisions, this));
    Reset();
  }
}

void
IncOrchestratorCommunicationTracer::SetAveragingPeriod(const Time& period)
{
  m_period = period;
  m_printEvent.Cancel();
  m_printEvent = Simulator::Schedule(m_period,
    &IncOrchestratorCommunicationTracer::PeriodicPrinter, this);
}

void
IncOrchestratorCommunicationTracer::PeriodicPrinter()
{
  Print(*m_os);
  Reset();
...
void
IncOrchestratorCommunicationTracer::PrintHeader(std::ostream& os) const
{
  os  << "Node,"
      << "ISent,"
      << "DReceived,"
      << "DecisionsSent,"
      << "OrchestratorInterestNames,"
      << "OrchestratorDecisions";
}

void
IncOrchestratorCommunicationTracer::Reset()
{
  m_stats.Reset();
}
...
void
IncOrchestratorCommunicationTracer::Print(std::ostream& os) const
{
...
void
IncOrchestratorCommunicationTracer::PrintSum(std::ostream& os) const
{
  std::stringstream ss;
...
// ---------------------------------------------------- //
void
IncOrchestratorCommunicationTracer::SendOrchestrationInterests(shared_ptr<const Interest> interest) {
...
void
IncOrchestratorCommunicationTracer::SendOrchestrationDecisions(shared_ptr<const Interest> interest) {
  m_stats.m_OrchestrationDecisionEnforced++;
  m_stats.m_overallOrchestrationDecisionsSend++;

  m_stats.m_orchestrationDecisions.push_back(interest->getName().toUri());
}

void
IncOrchestratorCommunicationTracer::IncomingOrchestrationDatas(shared_ptr<const Data> data){
  // int hopCount = 0;
