...
#ifndef SRC_INC_NODE_ORCHESTRATOR_TRACER_HPP_
#define SRC_INC_NODE_ORCHESTRATOR_TRACER_HPP_
...
/// @cond include_hidden
struct RequestStats_node_orch {
  inline void
  Reset()
  {
    m_enableFunctions = 0;
    m_disableFunctions = 0;
  }
  double m_enableFunctions=0;                         /// @brief counter to measure the functions enabled at a compute node
  double m_disableFunctions = 0;                      /// @brief counter to measure the functions disabled at a compute node
  std::vector<std::string> m_enabledFunctionsNames;   /// @brief list of function names enabled at the compute node
  std::vector<std::string> m_disabledFunctionsNames;  /// @brief list of function named disabled at the compute node
};
/// @endcond
 ...
 */
class IncOrchestratorTracerNode : public SimpleRefCount<IncOrchestratorTracerNode> {
public:
  /**
   ...
   * tuple needs to be preserved for the lifetime of simulation, otherwise SEGFAULTs are inevitable
   */
  static Ptr<IncOrchestratorTracerNode>
   ...
   */
  IncOrchestratorTracerNode(std::shared_ptr<std::ostream> os, Ptr<Node> node);
   ...
   */
  IncOrchestratorTracerNode(std::shared_ptr<std::ostream> os, const std::string& node);

  /**
   * @brief Destructor
   */
  ~IncOrchestratorTracerNode();
...
private:
  ...
  void
  EnableFunctionCounter(std::string);

  void
  DisableFunctionCounter(std::string);
...
private:
  ...
  RequestStats_node_orch m_stats;
};
 ...
 */
inline std::ostream&
operator<<(std::ostream& os, const IncOrchestratorTracerNode& tracer)
{
  os << "# ";
