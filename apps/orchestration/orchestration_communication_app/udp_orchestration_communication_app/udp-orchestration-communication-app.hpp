...
#ifndef UDP_ORCHESTRATOR_COMMUNICATION_APP_H
#define UDP_ORCHESTRATOR_COMMUNICATION_APP_H
...
/**
 *
 * \brief Application installed on orchestrator node to orchestrate compute nodes in a network
 *
 *
 */
class UdpOrchestrationCommunicationApp : public Application
{
public:
  /**
  ...
  UdpOrchestrationCommunicationApp ();

  virtual ~UdpOrchestrationCommunicationApp ();
  ...
  //send a temporary message, used for orchestrating nodes, does not change m_data
  void SendOnceWithFill(std::string fill);
  void SendOnce(void);
...
private:
  ...
  /// Callbacks for tracing the packet decision Tx events
  TracedCallback<Ptr<const Packet> > m_deciTxTrace;
  /// Callbacks for tracing the packet Rx events
  TracedCallback<Ptr<const Packet> > m_rxTrace;

  typedef void ( *TxTracedCallback)(shared_ptr<const Packet>);
  typedef void ( *RxTracedCallback)(shared_ptr<const Packet>);
  typedef void ( *DeciTxTracedCallback)(shared_ptr<const Packet>);
...
#endif /* INC_UDP_ORCHESTRATION_CLIENT_APP_H */
